...
GLRenderer::GLRenderer(melonDS::NDS& nds, Plugins::Plugin* plugin, bool compute)
    : Renderer(nds.GPU)
{
    AuxInputBuffer[0] = new u16[256 * 256];
    AuxInputBuffer[1] = new u16[256 * 192];

    Rend2D_A = std::make_unique<GLRenderer2D>(GPU.GPU2D_A, *this, plugin);
    Rend2D_B = std::make_unique<GLRenderer2D>(GPU.GPU2D_B, *this, plugin);

    // TODO, eventually: figure out a nicer way to support different 3D renderers?
    IsCompute = compute;
    if (IsCompute)
        Rend3D = std::make_unique<ComputeRenderer3D>(GPU.GPU3D, *this, plugin);
    else
        Rend3D = std::make_unique<GLRenderer3D>(GPU.GPU3D, *this, plugin);
}
