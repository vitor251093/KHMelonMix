...
GLRenderer3D::GLRenderer3D(melonDS::GPU3D& gpu3D, GLRenderer& parent, Plugins::Plugin* plugin) noexcept :
    Renderer3D(gpu3D), Parent(parent), GamePlugin(plugin), Texcache(gpu3D.GPU, TexcacheOpenGLLoader(false))
{
    ...
    for (int i = 0; i < npolys; i++)
    {
        ...
        if ((texparam != curtexparam) || (texpal != curtexpal))
        {
            ...
            if (TexEnable && (textype != 0))
            {
                ...
                {
                    u32* halp;
                    Texcache.GetTexture(texparam, texwidth, texheight, texpal, curtexid, curtexlayer, halp);
