...
ComputeRenderer3D::ComputeRenderer3D(melonDS::GPU3D& gpu3D, GLRenderer& parent, Plugins::Plugin* plugin)
    : Renderer3D(gpu3D), Parent(parent), GamePlugin(plugin), Texcache(gpu3D.GPU, TexcacheOpenGLLoader(true))
{}
...
{
    Texcache.GamePlugin = GamePlugin;
    ...
    for (int i = 0; i < GPU3D.RenderNumPolygons; i++)
    {
        ...
        if (!foundVariant)
        {
            ...
            if (enableTextureMaps && textype)
            {
                ...
                {
                    Texcache.GetTexture(polygon->TexParam, polygon->Width, polygon->Height, polygon->TexPalette, variant.Texture, prevTexLayer, textureLastVariant);
        ...
        GamePlugin->gpuOpenGL_applyChangesToPolygon(ScaleFactor, scaledPositions, polygon);
