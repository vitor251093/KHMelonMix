...
std::unique_ptr<GLRenderer> GLRenderer::New(melonDS::GPU& gpu, Plugins::Plugin* plugin) noexcept
{
    ...
    auto ret = std::unique_ptr<GLRenderer>(new GLRenderer(gpu, plugin));
...
GLRenderer::GLRenderer(melonDS::GPU& gpu, Plugins::Plugin* plugin)
        : Renderer2D(), GPU(gpu), GamePlugin(plugin)
{
    BackBuffer = 0;
